...
def get_sim_type(sim_reference, data_directory):
	"""
	...
	if sim_reference in ["permian", "early_carboniferous", "late_carboniferous"]:
		...
		fine_file = os.path.join(data_directory, "paleo_maps", "{}.tif".format(sim_reference))
...
if __name__ == "__main__":
	...
	save_dir = "/work/set114/Panama/Results/Paleo_06/"
	...
	sample_file, fine_file = get_sim_type(sim_type, data_dir)
	...
	for pause_file in paused_files_list:
		if not os.path.exists(os.path.join(save_dir, "Pause", (pause_file + str(job_num) +"_"+str(job_type)+".csv"))):
			is_pause_sim = False
	if is_pause_sim:
		...
		for pause_file in paused_files_list:
			pause_file_src = os.path.join(save_dir, "Pause", (pause_file + str(job_num) +"_"+str(job_type)+".csv"))
			pause_file_dst = os.path.join(output_dir, "Pause", (pause_file + str(job_num) +"_"+str(job_type)+".csv"))
