...
from paleo_occ_sq import get_sim_parameters, get_sample_size, get_sim_type, copyall
...
if __name__ == "__main__":
    ...
    global_save_dir = os.path.join(homedir, "Results/PaleoMainOcc/Clustered2")
    ...
    params = get_sim_parameters(pbs_index, global_save_dir, sim_type="clustered")
    seed = params["seed"]
    ...
    sample_file, fine_file, fragment_file = get_sim_type(
        interval, tetrapod_group, data_dir, fragment_dir, sim_type="clustered",
    )
    ...
    if is_pause_sim:
        ...
        simulation.resume_coalescence(
            pause_directory=output_dir, seed=seed, job_type=job_type, max_time=60 * 60 * 21.5, out_directory=output_dir
        )
    else:
        simulation.run()
    ...
    try:
        ...
        try:
            ...
            try:
                shutil.copy2(
                    speciation_file, os.path.join("/work/set114/Results/Dump/", "data_{}_{}.db".format(job_type, seed))
                )
            except Exception as ec:
                print("also failed...")
