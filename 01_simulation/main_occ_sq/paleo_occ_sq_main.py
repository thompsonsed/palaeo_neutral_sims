...
if __name__ == "__main__":
    ...
    global_save_dir = os.path.join(homedir, "Results/PaleoMainOcc/Main2")
    ...
    params = get_sim_parameters(pbs_index, global_save_dir, sim_type="pristine")
    seed = params["seed"]
    ...
    sample_file, fine_file, fragment_file = get_sim_type(
        interval, tetrapod_group, data_dir, fragment_dir, sim_type="pristine"
    )
    ...
    if is_pause_sim:
        ...
        shutil.copy(
            end_location,
            os.path.join(tmpdir, "Data", "data_{}_{}.db".format(str(job_type), str(seed))),
        )
