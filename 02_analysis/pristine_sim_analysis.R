...
data_dir <- "input"
results_dir <- file.path("results", "Sim5")
rdata_dir <- "rdata"
...
sim_results <- read.csv(file.path(results_dir, "results_.csv"))
...
summary_stats <- single_param_result  %>%
	...
	summarise(mean_value = mean(value),
		  ...
		  sd_value = sd(value),
		  total=n())
write.csv(summary_stats, file=file.path(results_dir, "summary_stats.csv"))
single_param_result <- single_param_result %>% ungroup() %>% top_n(n=5, wt=total_gof)
...
summary_stats_separate <- single_param_result_separate  %>%
	...
	summarise(mean_value = mean(value),
		  ...
		  sd_value = sd(value),
		  total=n())
write.csv(summary_stats_separate, file=file.path(results_dir, "summary_stats_separate.csv"))
...
save(single_param_result, file=file.path(rdata_dir, "pristine_best.RData"))
...
fragment_richness_rdata_path <- file.path(rdata_dir, "fragment_richness.RData")
if(file.exists(fragment_richness_rdata_path))
{
	load(fragment_richness_rdata_path)
}else{
	fragment_richness <- read_csv(file.path(results_dir, "results_fragment_abundances_.csv"),
...
pdf(file.path(fig_dir, "species_area_best.pdf"), 6, 3)
print(p4)
dev.off()
...
p_all <- single_param_result %>% gather(key="metric", value="value",
	...
	facet_grid(metric~tetrapod_group, scales = "free_y",
		   labeller = labeller(metric = as_labeller(c("alpha_diversity" = "Alpha diversity",
		   					   ...
		   					   "richness" = "Species richness"))))
print(p_all)
pdf(file.path(fig_dir, "main_metrics_all.pdf"),  6.5, 4)
print(p_all)
dev.off()
pdf(file.path(fig_dir2, "main_metrics_all.pdf"),  6.5, 4)
print(p_all)
dev.off()
...
summary_stats_late <- late_results  %>%
	...
	summarise(mean_value = mean(value),
		  ...
		  sd_value = sd(value),
		  total=n())
write.csv(summary_stats_late, file=file.path(results_dir, "summary_stats_late.csv"))
# Calculate the minimum goodness-of-fit in the top 5 fitting sims for each tetrapod group
...
p_all <- early_results %>% gather(key="metric", value="value",
	...
	facet_grid(metric~tetrapod_group, scales = "free_y",
		   labeller = labeller(metric = as_labeller(c("alpha_diversity" = "Alpha diversity",
		   					   ...
		   					   "richness" = "Species richness")))) +
	geom_vline(linetype="dashed", colour="black", aes(xintercept=307))
pdf(file.path(fig_dir, "main_metrics_early.pdf"),  6.5, 4)
print(p_all)
dev.off()

pdf(file.path(fig_dir2, "main_metrics_early.pdf"),  6.5, 4)
print(p_all)
dev.off()
...
p_all <- variable_results %>%
	...
	facet_grid(metric~tetrapod_group, scales = "free_y",
		   labeller = labeller(metric = as_labeller(c("alpha_diversity" = "Alpha diversity",
		   					   ...
		   					   "richness" = "Species richness")))) +
	geom_vline(linetype="dashed", colour="black", aes(xintercept=307))
pdf(file.path(fig_dir, "main_metrics_variable.pdf"),  6.5, 4)
print(p_all)
dev.off()

pdf(file.path(fig_dir2, "main_metrics_variable.pdf"),  6.5, 4)
print(p_all)
dev.off()
# p_all
#
...
pdf(file.path(fig_dir2, "paleo_parameter_space.pdf"), 4.5, 8.5)
print(p)
dev.off()
