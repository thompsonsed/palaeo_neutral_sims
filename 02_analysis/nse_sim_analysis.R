# Analyse the spatially implicit simulations assuming a well-mixed community of individuals at each
# time slice.
...
results_dir <- file.path("results", "nse1")
rdata_dir <- "rdata"
...
if(!file.exists(file.path(rdata_dir, "nse.RData")))
{
	nse_richness <- read.csv(file.path(results_dir, "nse_richness1.csv"))%>% select(-X) %>%
	...
	nse_metrics <- nse_richness %>% full_join(nse_alpha_beta) %>% full_join(real_biodiversity) %>%
	...
	save(nse_metrics, nse_grouped_parameters, top_simulations, best_simulations_richness,
	     file=file.path(rdata_dir, "nse.RData"))

}else
{
	load(file.path(rdata_dir, "nse.RData"))
}
	...
	facet_grid(metric~tetrapod_group, scales = "free_y",
		   labeller = labeller(metric = as_labeller(c("mean_alpha" = "Alpha diversity",
		   		    ...
		   		    tetrapod_group=as_labeller(c("amniote" = "Amniote",
		   		    			     "amphibian" = "Amphibian"))))
pdf(file.path(fig_dir, "nse_metrics_all.pdf"), 6, 5)
print(p_all)
dev.off()
